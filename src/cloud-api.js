...
async function fetchRemote(){
  ...
  const operation=(async()=>{
    try{
      ...
      if(!['student','admin'].includes(data.role)||data.role!==membership.role)throw Error('No se pudo verificar el permiso de esta cuenta.');
      cached=data.works||[];
...
async function login(email,password,expectedRole){
  ...
  if(mutating)throw Error('Espera a que termine el guardado antes de cambiar de cuenta.');
  if(session)await logout();
  email=String(email||'').trim();
  if(email.toUpperCase()==='ADMIN')email='quispeyuber06+admin@gmail.com';
  if(['T04128B','YUBER'].includes(email.toUpperCase()))email='quispeyuber06@gmail.com';
  emit({phase:'checking',role:'',email:'',lastSynced:'',message:'Comprobando tu cuenta…'});
  ...
  try{await fetchRemote();if(expectedRole&&membership.role!==expectedRole)throw Error(expectedRole==='admin'?'Esta cuenta es de estudiante. Usa la cuenta administradora.':'Esta cuenta es de administrador. Selecciona el acceso Administrador.');redraw();}catch(e){await logout();emit({message:explain(e)});throw e;}
}
